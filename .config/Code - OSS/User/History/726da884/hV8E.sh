...
is_browser_playing_video() {
    ...
    browser_window=$(hyprctl activewindow | grep -E "class: (Firefox|Chromium|Google-chrome|Brave|Vivaldi)")
    
    # Dodatkowa kontrola, aby sprawdzić, czy w tytule okna są typowe frazy związane z odtwarzaniem wideo
    # Flaga -i powoduje ignorowanie wielkości liter
    video_window=$(hyprctl activewindow | grep -i -E "title:.*(YouTube|Netflix|Player|Vimeo|Fullscreen)")
